...
# Newton solver:
def newt_solver(iters, x, J, RHS):
    camera = np.zeros([1, iters])
    for k in range(iters):
        camera[k] = x + np.invert(J)*RHS(x)
    return x


def jacobian(func,t,x,fx=None , eps=1.e-8):
    """
...
truck = Truck()
truck._initialise()
init_val = list(truck.initial_conditions())


print(jacobian(truck.fcn(1,[0]),0,init_val))
